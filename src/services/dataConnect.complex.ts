...
export async function createFullLearningPlanInDB(
    ...
    try {
        ...
        logger.error(`[Admin SDK Complex] Error creating full learning plan: ${errorMessage}`);
        throw error;
    }
}
...
export async function saveDailyContentDetailsInDB(dayContentId: string, llmContent: LlmDayContent): Promise<boolean> {
    try {
        ...
        if (!allOperationsSucceeded) {
            logger.error(`[Admin SDK Complex] Some operations failed while saving daily content: ${dayContentId}`);
        } else {
            logger.info(`[Admin SDK Complex] Daily content details saved successfully: ${dayContentId}`);
        }
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        logger.error(`[Admin SDK Complex] Error saving daily content details: ${errorMessage}`);
        throw error;
    }
}
